...
namespace Narciarze_v_2.Pages.Strefa_Administracji.Zarzad
{
    public class raporty_zarzad_Model : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            conn.Open();
            string query_karn = "SELECT  MONTH([Data_sprzedazy]) as mies, year([Data_sprzedazy]) as rok, sum(ck.Cena) as suma FROM Karnety k, Stoki as s, Cennik as c, Cena_karnety as ck WHERE s.ID = ck.ID_Stok AND c.ID_Cena_karnet = ck.ID and k.ID_Cennik=c.ID GROUP BY MONTH([Data_sprzedazy]), YEAR([Data_sprzedazy])";
            string query_bil = "SELECT  MONTH([Data_sprzedazy]) as mies, year([Data_sprzedazy]) as rok, sum(cb.Cena_przejazd * b.Ilosc_zjazdow) as suma  FROM Bilety as b, Cena_bilety as cb, Cennik as c, Wyciagi as w  where c.ID_Cena_bilet = cb.ID AND cb.ID_Wyciag = w.ID and b.ID_Cennik=c.ID GROUP BY MONTH([Data_sprzedazy]), YEAR([Data_sprzedazy])";
